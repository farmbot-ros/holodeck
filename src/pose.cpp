...
class Beacon {
  ...
  public:
    Beacon(rclcpp::Node::SharedPtr node) : node(node) {
        ...
        robo_pose = node->create_subscription<nav_msgs::msg::Odometry>("loc/odom", 10, std::bind(&Beacon::robo_pose_callback, this, _1));

        gps_pose = node->create_subscription<sensor_msgs::msg::NavSatFix>("loc/fix", 10, std::bind(&Beacon::robo_gps_callback, this, _1));
    }
  ...
  private:
    void robo_pose_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        //                                    rerun::Quaternion::from_wxyz(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x,
        //                                                                 msg->pose.pose.orientation.y, msg->pose.pose.orientation.z)));
        rec->log_static("world/map/" + namespace_, rerun::Points3D(points).with_colors(colors).with_radii({2.f}));

        // float delta = delta_distance({static_cast<float>(msg->pose.pose.position.x), static_cast<float>(msg->pose.pose.position.y), static_cast<float>(msg->pose.pose.position.z)}, positions.back());

        if (counter_pos % every == 0) {
            positions.push_back({x, y, z});
            auto linestrip = rerun::components::LineStrip3D(positions);
            rec->log_static("world/map/" + namespace_ + "/trajectory", rerun::LineStrips3D(linestrip).with_colors({colors}));
        }
    }
    ...
    void robo_gps_callback(const sensor_msgs::msg::NavSatFix::SharedPtr msg) {
        ...
        if (counter_loc % every == 0) {
            ...
            rec->log_static("world/map/" + namespace_ + "/trajectory", rerun::GeoLineStrings(linesting).with_colors({colors}));
        }
    }
